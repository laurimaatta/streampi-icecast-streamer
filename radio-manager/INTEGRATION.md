...
- **Behaviour**:
  - **SWITCH**: Radio Manager starts `darkice-gpio.service`, which runs `darkice_gpio.py`. The switch controls DarkIce (on = stream on, off = stream off).
  - **ON / OFF**: Radio Manager stops `darkice-gpio.service`. The web UI (or API) controls the stream; the switch has no effect.
